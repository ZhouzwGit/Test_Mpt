...
import com.mpt.hxqh.mpt_project.adpter.TransferLine_ScanAdapter;
...
public class TransferLine_Activity extends BaseActivity implements View.OnClickListener {
    ...
    private TransferLine_ScanAdapter transferLineScanAdapter;
    private int page = 1;
    ...
    protected void initView() {
        ...
        titleTextView.setText("Asset Transfer");
        buttonLayout.setVisibility(View.VISIBLE);
        layoutManager = new LinearLayoutManager(TransferLine_Activity.this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.snscan_button_id:
                Intent scanIntent = new Intent(TransferLine_Activity.this, MipcaActivityCapture.class);
                scanIntent.putExtra("mark", 1); //扫码标识
                startActivityForResult(scanIntent, TRANSFERLINE_CODE);
                break;
            case R.id.quit:
                final NormalDialog dialog = new NormalDialog(TransferLine_Activity.this);
                ...
                dialog.setOnBtnClickL(
                        ...
                        new OnBtnClickL() {
                            @Override
                            public void onBtnClick() {
                                AppManager.AppExit(TransferLine_Activity.this);
                            }
                        });
                break;
            case R.id.option:
                final NormalListDialog normalListDialog = new NormalListDialog(TransferLine_Activity.this, optionlist);
    ...
    private void initAdapter(final List<INVUSELINE> list) {
        ...
        transferLineScanAdapter = new TransferLine_ScanAdapter(TransferLine_Activity.this, R.layout.udretireline_item, list, status);
        recyclerView.setAdapter(transferLineScanAdapter);
        transferLineScanAdapter.setOnRecyclerViewItemChildClickListener(new BaseQuickAdapter.OnRecyclerViewItemChildClickListener() {
            @Override
    ...
    private void showEditDialog(final int position) {
        final NormalEditTextDialog dialog = new NormalEditTextDialog(TransferLine_Activity.this);
    ...
    private void getData() {
        HttpManager.getDataPagingInfo(TransferLine_Activity.this, HttpManager.getINVUSELINEURL(mainnum, page, 20, ""), new HttpRequestHandler<Results>() {
            @Override
            ...
            public void onSuccess(Results results, int totalPages, int currentPage) {
                ...
                if (item == null || item.isEmpty()) {
                    ...
                    if (page > totalPages) {
                        MessageUtils.showMiddleToast(TransferLine_Activity.this, getString(R.string.have_load_out_all_the_data));
                    } else {
    ...
    private void addData(final List<INVUSELINE> list) {
        transferLineScanAdapter.addData(list);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == TRANSFERLINE_CODE) {
            if (data != null) {
                ...
                isExistSN(results);
                Toast.makeText(TransferLine_Activity.this, results, Toast.LENGTH_SHORT).show();
            }
        }
    }

    public void isExistSN(final String result) {
        HttpManager.getDataPagingInfo(TransferLine_Activity.this, HttpManager.getINVUSELINEURL(mainnum, page, 1, result), new HttpRequestHandler<Results>() {
            @Override
            ...
            public void onSuccess(Results data, int totalPages, int currentPage) {
                ArrayList<INVUSELINE> item = JsonUtils.parsingINVUSELINE(data.getResultlist());
                if (item == null || item.isEmpty()) {
                    final NormalDialog normalDialog = new NormalDialog(TransferLine_Activity.this);
                    normalDialog.title("");
                    ...
                    if ("WHAPPR".equals(status)) {
                        if (item.get(0).getSCANSN() != null && item.get(0).getSCANSN().equals(item.get(0).getSERIALNUM())) {
                            Toast.makeText(TransferLine_Activity.this, "The SN is already scaned", Toast.LENGTH_SHORT).show();
                        } else {
    ...
    private void showConfirmDialog(final INVUSELINE invuseline) {
        final NormalDialog normalDialog = new NormalDialog(TransferLine_Activity.this);
        normalDialog.setTitle("");
                ...
                new OnBtnClickL() {
                    ...
                    public void onBtnClick() {
                        ...
                        for (int i = 0; i < items.size(); i++) {
                            if ((items.get(i).SERIALNUM==null?"":items.get(i).SERIALNUM).equals(invuseline.getSERIALNUM())) {
                                if ("WHAPPR".equals(status)) {
    ...
    public void saveUdRetireLine(final INVUSELINE invuseline) {
        new AsyncTask<String, String, String>() {

            @Override
            protected String doInBackground(String... strings) {
                String results = AndroidClientService.UpdateWO(TransferLine_Activity.this, invuseline.toString(), "INVUSELINE", "INVUSELINEID", invuseline.INVUSELINEID, Constants.WORK_URL);
                return results;
            }
            ...
            protected void onPostExecute(String s) {
                ...
                Toast.makeText(TransferLine_Activity.this, "成功".equals(s)?"Succeed":"Fail", Toast.LENGTH_SHORT).show();
            }
        }.execute();
    }
}
